...
export async function reviewDesign(researchPath: string): Promise<string> {
  ...
  const decision = await resolveLeadDesignerCase({ system: "Design Review", problem: `Review the proposed design contained in ${researchPath}. Determine what, if anything, is authorized for BLOODLINES.`, repositoryEvidence: `Research proposal path: ${researchPath}\nThe proposal itself is evidence, not approval.`, history: research });
